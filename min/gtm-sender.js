(function() {
  ...
  function fireContactInfoEvent() {
    ...
    log('Ecommerce: add_contact_info captured (' + bookingType + ')');

    getHashedUserData(function(hashedData) {
      sendToParent(addBookingTypeToEvent({
  ...
  function setupEcommerceListeners() {
    ...
    document.addEventListener('click', function(e) {
      ...
      if (target.matches('#showRatesBtn') || target.closest('#showRatesBtn')) {
        ...
        observeForElement({
          ...
          onFound: function(vehicles) {
            var bookingType = getBookingType();
            log('Ecommerce: view_item_list (' + vehicles.length + ' vehicles, ' + bookingType + ')');
            sendToParent(addBookingTypeToEvent({
              event: 'view_item_list',
              item_list_id: 'vehicles',
              item_list_name: bookingType === 'quote' ? 'Available Vehicles (Quote)' : 'Available Vehicles',
      ...
      if (vehicleBtn && !target.closest('#showRatesBtn')) {
        var bookingType = getBookingType();
        log('Ecommerce: select_item (' + bookingType + ')');
        var vehicleData = getVehicleData(vehicleBtn);
        sendToParent(addBookingTypeToEvent(Object.assign({ event: 'select_item' }, vehicleData)));
      }
      ...
      if (guestBtn || (target.tagName === 'BUTTON' && target.textContent.toLowerCase().includes('continue as guest'))) {
        var bookingType = getBookingType();
        log('Ecommerce: begin_checkout (' + bookingType + ')');
        ...
        getHashedUserData(function(hashedUserData) {
          var eventData = addBookingTypeToEvent(Object.assign({
    ...
    document.addEventListener('change', function(e) {
      if (e.target.matches('#PaymentTypeId')) {
        var bookingType = getBookingType();
        log('Ecommerce: add_payment_info (' + bookingType + ')');
